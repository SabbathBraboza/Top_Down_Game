...
public class CarController : MonoBehaviour
{
      ...
      void FixedUpdate()
      {
            ...
            {
                  rb.AddForce(transform.right * currentHorsePower);
            }
            else if (Input.GetKey(KeyCode.S))
            {
                  rb.AddForce(-transform.right * currentHorsePower / 2);
            }
            ...
            inDrift = driftFactor == driftSlippy;
            UpdateDrag();
      }
      ...
      private void UpdateDrag()
      {
            ...
            if (Input.GetKey(KeyCode.Space)) { rb.drag += 2; }
      ...
      private bool IsDrivingForward() => Vector2.Dot(transform.up.normalized, rb.velocity.normalized) >= 0;
}